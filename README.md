...
Unit-тесты хранятся в `test/unit/`.

Рекомендуемая структура тестов:

- `test/unit/domain/` — быстрые unit-тесты чистой доменной логики;
- `test/integration/` — интеграционные тесты, если они понадобятся;
- `test/fixtures/` — тестовые данные;
- `test/helpers/` — общие вспомогательные функции для тестов.
...
- `src/app/` — приложение на Next.js App Router: страницы, layout и провайдеры;
- `src/lib/` — инфраструктурные модули и общие утилиты, например Prisma Client;
- `src/components/` — общие переиспользуемые UI-компоненты;
- `src/domain/` — чистая бизнес-логика без зависимости от React, Next.js и Prisma;
- `src/use-cases/` — сценарии приложения, связывающие доменные правила с repository-интерфейсами;
- `src/styles/` — глобальные стили;
- `test/` — unit-тесты, будущие интеграционные тесты, тестовые данные и вспомогательные функции;
- `prisma/` — Prisma-схема, миграции и seed-скрипты;
- `data/` — исходные JSON-данные для начального наполнения;
- `documentation/` — продуктовая и техническая документация;
- `public/` — статические файлы;
- `.temp/` — рабочие планы, черновики и временные заметки.
